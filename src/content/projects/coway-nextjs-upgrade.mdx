...
tags: ['Next.js', 'React', 'Migration', 'Testing']
...
| react-i18next | v12.x | v14.x | React 19 지원 |
| framer-motion | v10.x | v11.x | ref props 변경 대응 |

### TypeScript 타입 에러 해결

```typescript
// @types/react v19 업데이트로 인한 타입 변경
...
**수동 테스트 항목**:
- [ ] 모든 서비스 페이지 동작 (API 요청 포함)
- [ ] 디바이스 상태 실시간 업데이트 (WebSocket)
- [ ] 네이티브 브릿지 통신 (iOS/Android)
- [ ] 커스텀 바텀시트 동작 (framer-motion 관련 로직 수정)
- [ ] 에러 바운더리 처리

**결과**: 주요 기능 모두 정상 동작 확인
