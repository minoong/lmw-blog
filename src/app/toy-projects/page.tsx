      ...
      {projects.length > 0 ? (
        <div className="space-y-8">
          {projects.map((project) => (
            <div
              key={project.slug}
              className="dark:border-claude-border dark:bg-claude-surface group relative rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
            >
              <div className="mb-4 flex items-start justify-between">
                <Link href={`/toy-projects/${project.slug}`} className="after:absolute after:inset-0">
                  <h2 className="text-2xl font-bold transition-colors group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400">
                    {project.title}
                  </h2>
                </Link>
                ...
                <div className="flex gap-2">
                  {project.github && (
                    <a
                      ...
                      className="relative z-10 text-gray-600 transition-colors hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400"
                      aria-label="GitHub"
                    >
