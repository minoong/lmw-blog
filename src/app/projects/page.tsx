      ...
      {projects.length > 0 ? (
        <div className="space-y-8">
          {projects.map((project) => (
            <div key={project.slug} className="rounded-xl border border-gray-200 bg-white p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl dark:border-gray-700 dark:bg-gray-800">
              <div className="mb-4 flex items-start justify-between">
                <h2 className="text-2xl font-bold dark:text-white">{project.title}</h2>
                <div className="flex gap-3">
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                      GitHub →
                    </a>
                  )}
                  {project.link && (
                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                      Live Demo →
                    </a>
                  )}
                </div>
        ...
        <div className="rounded-xl bg-gray-100 py-12 text-center dark:bg-gray-800">
          ...
          <p className="text-gray-400 dark:text-gray-500">
            <code className="rounded bg-gray-200 px-2 py-1 dark:bg-gray-700">src/content/projects/</code> 폴더에 MDX 파일을 추가하여 프로젝트를 등록할 수 있습니다.
          </p>
        </div>
      )}
    </div>
