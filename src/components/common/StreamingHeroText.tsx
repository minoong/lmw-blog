...
export default function StreamingHeroText({ lines, className = '', streamingSpeed = 20 }: StreamingHeroTextProps) {
  ...
  const Badge = () => (
    <span className="pointer-events-none relative -top-1 ml-1 inline-flex shrink-0 items-center align-middle">
      <svg className="h-5 w-5 animate-pulse text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        <path
          strokeLinecap="round"
      ...
      <span
        className="absolute -right-8 rounded bg-linear-to-r from-blue-500/70 to-purple-500/70 px-1 py-0.5 text-[8px] font-bold whitespace-nowrap text-white/90 shadow-sm"
